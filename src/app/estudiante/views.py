...
def asistencia_alumnos_list(request):
	...
	print(request.user)
	if request.user.es_estudiante and not request.user.is_anonymous():
		...
		# hijo_defecto = Estudiante.objects.get(user__username=request.user.username)
		if request.method == 'GET':
			fecha =  request.GET.get('fecha')
			# apoderado = Apoderado.objects.get(user=request.user)
			estudiante = Estudiante.objects.get(user__username=request.user.username)
			try:
				...
				return render(request, 'estudiante/asistencias/asistencia_alumnos_list.html', context)
...
def calificaciones_index(request):
	...
	if request.user.es_estudiante and not request.user.is_anonymous():
		...
		return render(request, 'estudiante/calificaciones/calificaciones.html', context)
	else:
		return redirect('/')
...
def calificaciones_alumnos_list(request, pk):
	...
	if request.user.es_estudiante and not request.user.is_anonymous():
		...
		'calificaciones_active':'active'}
		return render(request, 'estudiante/calificaciones/calificaciones_alumnos_list.html', context)
	else:
		return redirect('/')
...
# 		'apoderado':apoderado}
# 		return render(request, 'estudiante/observaciones/observaciones_alumnos_list.html', context)
# 	else:
# 		return redirect('/')
...
def tareas_alumnos_list(request):
	...
	if request.user.es_estudiante and not request.user.is_anonymous():
		...
		return render(request, 'estudiante/tareas/tareas_alumnos_list.html', context)
	else:
		return redirect('/')
